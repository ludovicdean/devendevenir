...
showCreditPhoto: true
---

La création de ce blog m'a dans un premier temps fait découvrir le compilateur de site web statique Jekyll ([voir cet article](localhost:4321/blog/jekyll/)).
La phase de découverte a été assez longue, et j'ai été amené à rencontrer des difficultés liées à combiner du CSS personnalisé à l'utilisation de Bootstrap 5.
Les difficultés croissantes rencontrées, malgré les fonctionnalités intéressantes, notamment la gestion des catégories, m'a amené à tester un autre outil, Astro.
...
Avant de vous initier à Astro, vous allez avoir besoin d'installer **NodeJS** (si ce n'est pas déjà fait 😊). Si tel n'est pas le cas, vous pouvez vous rendre à [cette adresse](https://nodejs.org/en/download/current) pour télécharger l'installateur qui correspond à votre système d'exploitation.
...
Vous avez effectué le plus gros du travail, tout se passe maintenant dans votre terminal :
...
- Saisissez `npm astro@latest`, validez puis saisissez `y` pour installer `create-astro`.
...
À première vue notre projet Astro contient peu de choses, quelques répertoires :

- node_modules
- public
- src

Ainsi que quelques fichiers à la racine du projet :
...
Détaillons le rôle et le contenu de ces différents éléments.
...
Le fichier package-lock.json est quant à lui un instantané de ce qui a été installé lors de la dernière installation du projet, lors du dernier `npm install`.
Contrairement au package.json, avec lequel npm va pouvoir installer des versions plus récentes de certaines dépendances, le fichier lock va lui forcer à installer les versions exactes de l'ensemble de ces dépendances, qu'il existe de nouvelles versions de celles-ci ou non.
...
Astro intègre nativement le mdx dans ces projets, mais de quoi s'agit-il ?

Le mdx (Markdown with JSX) est en quelque sorte le grand frère du md (Markdown). Il apporte de nombreuses fonctionnalités liées au JSX (JvaScript XML) :
...
Chaque article possède un frontmatter dans lequel sont valorisés des attributs qui sont utilisés à l'extérieur de l'article, pour l'affichage sur la page d'accueil, pour les liens intégrés aux crédits photographiques etc. On peut voir ici comment sont utilisées les informations du frontmatter pour afficher la liste des articles.
...
- composant head
- composant header
- contenu de l'article (contenu de votre fichier .mdx)
- composant crédits photographiques (alimenté par le frontmatter du fichier mdx)
- footer
...
Ici, si showCreditPhoto vaut true, alors on affiche le composant CreditPhoto en lui passant les paramètres nécessaires.
...
Astro m'a été vanté par un collègue au fait de mon projet de blog, notamment par rapport à sa simplicité. Mon expérience avec Astro est véritablement réussie, la prise en main de l'outil étant assez rapide, avec une courbe d'apprentissage assez douce. On obtient rapidement un résultat intéressant sans avoir véritablement mis les mains dans le cambouis.
